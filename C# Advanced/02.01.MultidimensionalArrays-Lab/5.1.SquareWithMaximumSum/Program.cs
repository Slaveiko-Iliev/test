...
int rowsOfBiggest = int.Parse(Console.ReadLine());
int colsOfBiggest = int.Parse(Console.ReadLine());

int biggest = int.MinValue;
int rowOfBest = int.MinValue;
int colOfBest = int.MinValue;

for (int row = 0; row < matrix.GetLength(0) - (rowsOfBiggest - 1); row++)
{
    for (int col = 0; col < matrix.GetLength(1) - (colsOfBiggest - 1); col++)
    {
        int currentSum = 0;
